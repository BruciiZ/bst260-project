...
Our analysis uses several analytical metrics mainly Case Fatality Rate (CFR), Hospitalization Rate (HR) and Death Proportion (DP). The CFR is defined as deaths per case, HR is defined as hospitalizations per case, and DP is defined as deaths per population.
...
top5_wave1 <- wave1_excess %>% arrange(desc(excess_death_rate)) %>% slice_head(n=5) %>% mutate(wave = "Wave 1", rank = row_number())
bottom5_wave1 <- wave1_excess %>% arrange(excess_death_rate) %>% slice_head(n=5) %>% mutate(wave = "Wave 1", rank = 53 - row_number())

top5_wave2 <- wave2_excess %>% arrange(desc(excess_death_rate)) %>% slice_head(n=5) %>% mutate(wave = "Wave 2", rank = row_number())
bottom5_wave2 <- wave2_excess %>% arrange(excess_death_rate) %>% slice_head(n=5) %>% mutate(wave = "Wave 2", rank = 53 - row_number())

top5_wave3 <- wave3_excess %>% arrange(desc(excess_death_rate)) %>% slice_head(n=5) %>% mutate(wave = "Wave 3", rank = row_number())
bottom5_wave3 <- wave3_excess %>% arrange(excess_death_rate) %>% slice_head(n=5) %>% mutate(wave = "Wave 3", rank = 53 - row_number())
